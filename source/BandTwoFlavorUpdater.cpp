/*
 * BandTwoFlavorGlueballMeasure.cpp
 *
...
BandTwoFlavorUpdater::BandTwoFlavorUpdater() : action(0), gaugeAction(0), fermionAction(0), diracOperator(0), bandAction(0), bandFermionAction(0), bandGaugeAction(0) { }

BandTwoFlavorUpdater::~BandTwoFlavorUpdater() {
	if (action) delete action;
	if (bandAction) delete bandAction;
	if (bandFermionAction) delete bandFermionAction;
	if (bandGaugeAction) delete bandGaugeAction;
}

void BandTwoFlavorUpdater::execute(environment_t& environment) {
	//Get the solver for the inversions
	BiConjugateGradient* solver = new BiConjugateGradient();
	...
	//Get the band action
	if (bandAction == 0) {
		bandAction = new BandAction(action);
		bandFermionAction = new BandAction(fermionAction);
		bandGaugeAction = new BandAction(gaugeAction);

		bandAction->addBand(0,4);
		bandFermionAction->addBand(0,4);
		bandGaugeAction->addBand(0,4);

		bandAction->addBand(8,12);
		bandFermionAction->addBand(8,12);
		bandGaugeAction->addBand(8,12);
	}

#ifdef DEBUGFORCE
	//Test of the force
	TestForce testForce;
	fundamental_lattice_t tmp;
	...
	if (numbers_steps.size() == 1) {
		forces.push_back(bandAction);
	} else if (numbers_steps.size() == 2) {
		forces.push_back(bandFermionAction);
		forces.push_back(bandGaugeAction);
	}
	else {
		std::cout << "BandTwoFlavorHMCUpdater::Warning, two flavor does not support more than two multiple time integration!" << std::endl;

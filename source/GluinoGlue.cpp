				...
				for (int c = 0; c < diracVectorLength; ++c) {
					...
					for (unsigned int i = 0; i < 3; ++i) {
						for (unsigned int j = 0; j < 3; ++j) {
							eta[site][mu][c] += Sigma::sigma(i,j,alpha,mu)*trace(cloverPlaquette(environment.gaugeLinkConfiguration,site,i,j)*tau.get(c));
						}
					}
					if (Layout::globalIndexT(site) == 0) {
						source[site][mu][c] = eta[site][mu][c];
					}
					else {
						source[site][mu][c] = 0.;
					}
...
GaugeGroup GluinoGlue::cloverPlaquette(const extended_gauge_lattice_t& lattice, int site, int mu, int nu) {
	...
	GaugeGroup final = std::complex<real_t>(+1./8.,0)*(result-htrans(result));
	return final;
}

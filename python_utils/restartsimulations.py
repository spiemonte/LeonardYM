...
patternoutputname = re.search('output_configuration_name\ *=\ *(.+)',filedata)
...
if numberLastConf == 0:
        print "Warning, no old configurations found, trying with new su3 format"
        for namefile in os.listdir(patternoutput.group(1)):
		pattern = re.search(patternoutputname.group(1)+'\.config\-([0-9]+)',namefile)
                if pattern != None:
                        tmp = int(pattern.group(1))
                        if tmp > numberLastConf:
                                numberLastConf = tmp
